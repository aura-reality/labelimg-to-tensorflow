...
def main(_):
  ...
  labels_list = list(os.listdir(labels_dir))
  for idx, label_file in enumerate(labels_list):
    if idx % 100 == 0:
      print("On image %d of %d" %  (idx, len(labels_list)))
    ...
    try:
      ...
      raise e
    label_data = dataset_util.recursive_parse_xml_to_dict(xml)['annotation']

    tf_example = dict_to_tf_example(label_data, images_dir, label_map_dict, FLAGS.ignore_difficult_instances)
